# Generated with JReleaser 1.16.0 at 2025-04-11T21:23:40.28121Z
FROM azul/zulu-openjdk-alpine:11-jre

    LABEL "org.opencontainers.image.title"="Tessellate"
    LABEL "org.opencontainers.image.description"="Tessellate is tool for parsing and partitioning data."
    LABEL "org.opencontainers.image.url"="https://github.com/ClusterlessHQ"
    LABEL "org.opencontainers.image.licenses"="MPL-2.0"
    LABEL "org.opencontainers.image.version"="1.0-wip-72"
    LABEL "org.opencontainers.image.revision"="2422a9d4fc8a8ae161c1e2607475c605fc65c215"


COPY assembly/* /

RUN unzip tessellate-1.0-wip-72.zip && \
rm tessellate-1.0-wip-72.zip && \
chmod +x tessellate-1.0-wip-72/bin/tess && \
mv /tessellate-1.0-wip-72 /tess

ENV PATH="${PATH}:/tess/bin"


ENTRYPOINT ["/tess/bin/tess"]

CMD ["--help"]
