# Generated with JReleaser 1.16.0 at 2025-05-02T23:10:07.116286Z
FROM azul/zulu-openjdk-alpine:11-jre

    LABEL "org.opencontainers.image.title"="Tessellate"
    LABEL "org.opencontainers.image.description"="Tessellate is tool for parsing and partitioning data."
    LABEL "org.opencontainers.image.url"="https://github.com/ClusterlessHQ"
    LABEL "org.opencontainers.image.licenses"="MPL-2.0"
    LABEL "org.opencontainers.image.version"="1.0-wip-79"
    LABEL "org.opencontainers.image.revision"="e7fa0791af3ea7ad755f474fd7c44bdbf6257d9c"


COPY assembly/* /

RUN unzip tessellate-1.0-wip-79.zip && \
rm tessellate-1.0-wip-79.zip && \
chmod +x tessellate-1.0-wip-79/bin/tess && \
mv /tessellate-1.0-wip-79 /tess

ENV PATH="${PATH}:/tess/bin"


ENTRYPOINT ["/tess/bin/tess"]

CMD ["--help"]
